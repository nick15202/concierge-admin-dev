 ...
 var FormioResourceService = /** @class */ (function () {
      ...
      */
         function (resource) {
             ...
             if (formio && formio.url === 'https://ooba-digitaloffice.form.io/notes') {
                 resource.data.contact = JSON.parse(resource.data.contact);
             }
 
             
             debugger;
             if (formio && formio.url === 'https://ooba-digitaloffice.form.io/matchLink') {
                for (let i = 0; i < resource.data.listings.length; i++) {
                    if(resource.data.listings[i].feedbackForm && resource.data.listings[i].feedbackForm._id){
                        const feedbackForm = {
                            '_id': resource.data.listings[i].feedbackForm._id
                        };
                        const scheduleForm = {
                            '_id': resource.data.listings[i].scheduleForm._id
                        };
                        resource.data.listings[i].feedbackForm = feedbackForm;
                        resource.data.listings[i].scheduleForm = scheduleForm;
                    }
                }
                
                 resource.data.listings.forEach(element => {
                     // if (element.listingId && element.listingId.length > 0) {
                     //     element.listingId = parseFloat(element.listingId.join());
                     // }
 
 
                     if (element.listingId == "" || element.listingId) {
                         element.listingId = parseFloat(Math.floor(100000 + Math.random() * 900000));
                         element.status = "status" + parseFloat(Math.floor(100000 + Math.random() * 900000));
                     }
                 });
             }
             return formio
     ...
     /** @nocollapse */
     FormioResourceService.ctorParameters = function () {
         return [
